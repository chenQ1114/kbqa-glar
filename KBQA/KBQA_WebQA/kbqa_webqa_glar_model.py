...
class KBQA_Webqa_GLARModel(nn.Module):
    def __init__(self, opt, emb_vs, vocab_id2word, padding_idx=0, state_dict=None):
        super(KBQA_Webqa_GLARModel, self).__init__()
        self.opt = opt
    ...
    def forward(self, data, only_get_fts=False, adv_training=False, given_fts=False):
        ...
        q_output = self.rep_rnn_q(q_embs)
        rel_output= self.rep_rnn_rel(rel_embs)
        #print('sru size'+str(q_output[0].size())+'||'+str(q_output[1].size())) [64, 10, 300]|| [1, 10, 300]

        # q_hidden_states = torch.cat(q_output[1:], 2)
        # rel_hidden_states = torch.cat(rel_output[1:], 2)

        q_hidden_states = q_output
        rel_hidden_states = rel_output
    ...
    def update(self, batch):
        ...
        self.train_loss.update(loss.data, len(q_idxs))
        ...
        return acc.data, score_cost.view(-1).data.cpu().numpy()
