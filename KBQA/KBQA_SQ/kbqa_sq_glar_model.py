...
class KBQA_SQ_GLARModel(nn.Module):
    ...
    def forward(self, data, only_get_fts=False, adv_training=False, given_fts=False):
        ...
        merge_in = torch.cat([merge_in, rel_match_states * q_hidden_states], dim=2) #merge_in size:[64, 14, 6 * q_hidden]
        merge_in = self.merge_liner(merge_in) # merge_in size: [64,14,6 * q_hidden]

        merge_in = F.relu(merge_in)
        merge_in = torch.cat([merge_in, rel_match_states, q_hidden_states], dim=2) #merge_in size: [64,14,10 * q_hidden]
        merge_res = self.merge_liner2(merge_in) #merge_res size [64,14,2 * merge_fts_dim]
        predicate = self.merge_liner3(rel_predicate_match_states)
        #rel_predicate_match_states: [64,8, 2 * q_hidden];   predicate size: [64,8, 2 * merge_fts_dim]
        ...
        probs = self.cosine(fts_q, fts_p) #probs size: [64]
        probs = probs.view(shallow_probs.size())

        shallow_probs = F.sigmoid(shallow_probs)

        merge_probs = torch.cat([probs, shallow_probs], dim=1)# [64,2]
        out = self.prob_out_linear(merge_probs)  #[64,1]
    ...
    def update(self, batch):
        ...
        if torch.cuda.is_available():
            q_idxs, g_rel_idxs, cdt_rel_idxs, g_knn_idxs, neg_knn_idxs, q_knn_idxs = [
                Variable(torch.from_numpy(e).long().cuda()) for e in
                (q_idxs, g_rel_idxs, cdt_rel_idxs, g_knn_idxs, neg_knn_idxs, q_knn_idxs)]

        else:
            q_idxs, g_rel_idxs, cdt_rel_idxs, g_knn_idxs, neg_knn_idxs, q_knn_idxs = [
                Variable(torch.from_numpy(e).long()) for e in
                (q_idxs, g_rel_idxs, cdt_rel_idxs, g_knn_idxs, neg_knn_idxs, q_knn_idxs)]
    ...
    def predict_score_of_batch(self, batch):
        with torch.no_grad():
            self.eval()
            if torch.cuda.is_available():
                q_idxs, rel_idxs, r_knn_idxs, q_knn_idxs = [
                    Variable(torch.from_numpy(e).long(), volatile=True).cuda() for e in batch]

            else:
                q_idxs, rel_idxs, r_knn_idxs, q_knn_idxs = [Variable(torch.from_numpy(e).long(), volatile=True) for e in
                                                            batch]
