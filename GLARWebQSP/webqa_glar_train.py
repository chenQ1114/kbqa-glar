...
from GLARWebQSP.webqa_parameters import JointWebqaParameters as JointWebqaParameters
from GLARWebQSP.webqa_DataManager import GLARWebqaDataManager
from utils import AverageMeter
from GLARWebQSP.websq_glar_model import GLARWebqaModel
...
class NerTrain:
    def __init__(self, opt=JointWebqaParameters(train_idx='none', dev=dev)):
        ...
        if self.opt.resume_dsrc_flag:
            ...
            self.model = GLARWebqaModel(opt=self.opt, emb_vs=self.dt.vocab.vs, vocab_id2word = self.dt.vocab._id_to_word)
        self.model.cuda()
    ...
    def resume(self):
        ...
        model = GLARWebqaModel(opt=self.opt, emb_vs=self.dt.vocab.vs, vocab_id2word = self.dt.vocab._id_to_word)  # 重启的时候
...
if __name__ == '__main__':
    opt = JointWebqaParameters(dev, train_idx='webqsp_glar')
    t = NerTrain(opt=opt)
    # t.valid_it(-1)
    t.train()
