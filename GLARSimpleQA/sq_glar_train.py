...
from GLARSimpleQA.SimpleQAParapmeters import RDParameters as JointWebqaParameters
from GLARSimpleQA.sq_DataManager import GLARSimpleQADataManager
from utils import AverageMeter
from GLARSimpleQA.sq_glar_model import SQ_GLARModel
...
class NerTrain:
    def __init__(self, opt=JointWebqaParameters(train_idx='none', dev=dev)):
        ...
        if self.opt.resume_dsrc_flag:
            ...
            self.model = SQ_GLARModel(opt=self.opt, emb_vs=self.dt.vocab.vs, vocab_id2word = self.dt.vocab._id_to_word)
        self.model.cuda()
    ...
    def resume(self):
        ...
        model = SQ_GLARModel(opt=opt, emb_vs=self.dt.vocab.vs, state_dict=state_dict, wh_idxs=self.dt.whs_idxs)  # 重启的时候
...
if __name__ == '__main__':
    opt = JointWebqaParameters(dev=dev, train_idx='sq_glar')
